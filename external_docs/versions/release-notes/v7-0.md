...
- **Rectangle card**, bigger visual with an horiontal layout
...
### Multiple tab search in CodeEditor
...
> for each mentionned resource, you can add `*_DARK` or `*_LIGHT` if you want to specify which theme base to use it on.
...
It is now possible to define the preferred format int the module's settings, e.g.:
...
These settings are used in the _Commit_  dialog to set the 2 corresponding fields that are made read only in this case. This ensures consistency
of format across commits.
