...
The internal additional field `row_metaobject_id` has been removed, the meta-object field support the link itself (through a refSearchSpec):
...
- Full export supports links through meta objects (ex: to export Permissions with a parent object)
...
- Added a **type of View** to separate through inheritance:
...
- **Permalink** to expose user-friendly URL:
  ...
  - a permalink uses a kebab-case syntax and is wrapped through the new servlet `/ui/l/<permalink URI>`
...
- Robustness of hook calls:
  ...
  - New hooks `hookBegin`, `hookEnd`, `methodBegin`, `methodEnd` to customize behavior (cf [trace hooks](/docs/core/hooks/businessobject-code-hooks#trace-hooks)):
...
- Backported tree view opens the main instance of object to allow edit list
...
This is the **final** revision for this minor version 6.0, no further maintenance will be done on it.
