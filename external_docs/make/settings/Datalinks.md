...
Available as of **Simplicité® version 5.3**, the **Data Link** feature allows automatic synchronization of object data (tables)
between multiple Simplicité® instances. Once linked, these remote objects behave as **local business objects**, supporting features like:
...
:::warning
Each network must include at least one **primary**. There can be multiple.
:::
...
- In extreme scenarios (e.g. broken DB or external SQL access corruption), disable the cron and **manually copy the data**
  between databases using direct SQL — ensure column integrity and `row_id` consistency
