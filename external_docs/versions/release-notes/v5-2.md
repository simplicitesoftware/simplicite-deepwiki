...
- New ability to rename table/column when installing a module with altered object's or field's name
  ...
  - No more manual migration needed through SQL
...
- Backported "no search on list" through menu access
...
- Backported new type of `Action` to launch long running job started through UI in a `isolated session`
...
- Fixed `URI_MAPPINGS` and `HTTP_HEADERS` reloading after module import
...
- Backported unit tests execution through import specification
...
- Backported ping platform nodes to clean `m_pf_node` periodically through the cron HealthCheck
...
This is the **final** revision for this minor version 5.2, no further maintenance will be done on it.
