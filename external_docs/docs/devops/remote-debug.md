...
:::warning
Starting your server in remote debug mode (JPDA with default socket connection mode) has a **very** significant negative
...
The server's debug port must be accessible from the local machine running the debugger. For remote instances,
this can be achieved by either opening the port in the server's firewall or, preferably,
by setting up an SSH tunnel to securely forward the port.
