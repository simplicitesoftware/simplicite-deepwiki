...
- The deprecated `PlatformHooks.isMenuEnable` has been removed.
  See [minor version 6.0 release note](/versions/release-notes/v6-0#core-changes) for refactoring instructions.
...
- No more internal EJB support to access the database:
  ...
  - The webapp connects the database through a monitored connections pool (JDBC),
...
- Added `AuditTool.checkDeadLinks` to detect dead links in logs and generate a SQL patch through audit tool:
...
- Allows update on simple referenced fields (not userkey) through the parent list or form
...
Example: set the Stock of Product updatable through the Order (by form, edit list and/or cell)
...
- Backported [changes on scope of `ObjectCore.(re)setAllFields*` helper methods](/versions/release-notes/v6-3#changed-scope-of-helper-methods)
...
- Backported [from 6.3](/versions/release-notes/v6-3#platform--module-version-check) major version check when importing a module:
  importing modules exported from
...
This is the **final** revision for this minor version 6.1, no further maintenance will be done on it.
