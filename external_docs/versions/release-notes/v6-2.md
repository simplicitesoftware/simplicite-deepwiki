...
- Backported [from v6.3](/versions/release-notes/v6-3#child-objects-change-logs) `Link.setMergeRedologs(true)`
  to retrieve child redologs in parent panel `Change logs`
- Backported [from v6.3](/versions/release-notes/v6-3#improvements-on-external-objects) UI external objects
  class metadata + `getResourceURL` and `service` functions
- Backported [from v6.3](/versions/release-notes/v6-3#ocr-library) integration of tesseract.js JS lib for basic client-side OCR
...
- Backported [from v6.3](/versions/release-notes/v6-3#new-helper-methods) the `ServletTool.success` helper method
- Backported [from v6.3](/versions/release-notes/v6-3#custom-health-check) the `customHealthCheck` platform hook for custom health check
- Backported [from v6.3](/versions/release-notes/v6-3#improved-openapiswagger-schemas) improvements on OpenAPI/Swagger schemas
...
- Backported [from v6.3](/versions/release-notes/v6-3#environment-variable-based-configurability) `[ROOT]` and `[URL]` tokens in expressions
- Backported [from v6.3](/versions/release-notes/v6-3#custom-disposition-simplification) implicit rewrite from `*/` to `*/index.html` in static contents
- Fixed audit widget to show only the "Start audit" button
- Backported [from v6.3](/versions/release-notes/v6-3#improved-openapiswagger-schemas) improvements on OpenAPI/Swagger schemas of generic APIs
- Backported [from v6.3](/versions/release-notes/v6-3#improved-mapped-api-configuration) improvements on OpenAPI/Swagger schemas of generic APIs
- Backported [from v6.3](/versions/release-notes/v6-3#development-mode) restrictions in development mode
- Fixed missing flush in API grants' object pools when clearing cache of a single business object or external object
- Backported [from v6.3](/versions/release-notes/v6-3#changed-scope-of-helper-methods) changes on scope of `ObjectCore.(re)setAllFields*` helper methods
- Fixed extend button of UI monitoring

### 6.2.5 (2025-03-28) {#version-6.2.5}

- Backported [from 6.3](/versions/release-notes/v6-3#platform--module-version-check) major version check when importing a module:
  importing modules exported from a higher major versions is now denied as it may have unexpected results
- Fixed meta-object cascade delete without explicit list
- Backported [from v6.3](/versions/release-notes/v6-3#mermaid-widget) the Mermaid widget
- Backported [from v6.3](/versions/release-notes/v6-3#improved-annotations) annotations improvements
...
- Backported [from v6.3](/versions/release-notes/v6-3#mail-service-configuration) support for `MAIL_SERVICE` overriding by the same name environment variable
...
- Backported [from v6.3](/versions/release-notes/v6-3#go-to-definition-generalization) generalization of "go to definition" icons
...
- Backported [from v6.3](/versions/release-notes/v6-3#environment-variable-based-configurability) `[ENV:<environment variable name>]` substitutions
...
- Added mention of the the `origin` module remote Git URI (if defined) to avoid ambiguities vs the instance's module Git URI.
  Note that in [v6.3](/versions/release-notes/v6-3#module-git) this legacy custom page is replaced by a dedicated business object exposing the same features
...
- Backported [from v6.3](/versions/release-notes/v6-3#custom-logout-confirmation-page) the `customLogoutConfirmPage` new platform hook
...
- Backported [from v6.3](/versions/release-notes/v6-3#new-helper-methods) new `Tool.before/after` date comparison shorthand methods
...
- Backported [from v6.3](/versions/release-notes/v6-3#copypb) cascade copy of pillbox links
...
- Security fixes
- Backported [from v6.3](/versions/release-notes/v6-3#development-mode) additional restrictions in development mode
- Backported [from v6.3](/versions/release-notes/v6-3#javascript-console-tool) the `$console` global variable (as an alias to the
...
- Updated Java libs
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.0-beta-RC2) addition
  of missing `com.simplicite.util.engine.PlatformHooksInterface` in public classes
  and removed legacy backward-compatibility class `com.simplicite.util.PlatformHooksInterface`
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.0-beta-RC2) fix to force a metadata reloading for UI instances with a parent object
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.0-beta-RC2) fix create button on list activity to use the hook `isCreateEnable`
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.0-beta-RC2) fix on environment variables
  and/or JVM properties substitutions in module's settings

### 6.2.21 (2026-01-15) - maintenance revision {#version-6.2.21}

- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.0-beta-RC5) fix for "Readonly form" contextual helps not displaying
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.0-beta-RC5) fix `displayForm` parent object context of inlined object
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.0) fix for inlined forms via a virtual link not using the SQL filter

### 6.2.22 (2026-02-06) - maintenance revision {#version-6.2.22}

- Backported [from v6.3](/versions/release-notes/v6-3#jclouds-openstack-swift) deprecation warnings for OpenStack swift cloud storage operations
...
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.1) fix for inlined objects displayed as tabs showing default templates
...
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.2) fix on counter widgets layout issues
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.3) fix on datalink synchronization of notepad field with textual rendering
...
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.4) fix on cascade copy of multi-documents field.
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.4) fix inlined 0,1/1,1 objects cascade copy and parent data reloading.
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.4) fix empty form of home instance of a specific "select" object.
...
- Backported [from v7.0](/versions/release-notes/v7-0#outdated-apache-commons) removal of the `xml-apis` lib (conflicting with JVM module).
...
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.7) fix on `grant.rollbackTransaction`.

### 6.2.26 (2026-04-14) - final maintenance revision {#version-6.2.26}
