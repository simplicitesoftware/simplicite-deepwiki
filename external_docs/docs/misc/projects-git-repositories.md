...
Starting with version 3.2 the configuration modules can be exposed as Git repositories (managed by the Simplicité instance itself).
...
The instances templates packages usage is straightforward: it contains a preinstalled Tomcat&reg; webapp
(using, by default, and embedded HSQLDB&reg; database) that can be deployed as-is.
...
2. Copy the content of the setup to a new Java project (avoid working with the setup repository itself
   unless you are a Git advanced user and are able to configure dedicated custom branches and remotes)
3. Setup the project using `ant setup` ANT task
4. If needed adjust manually the resulting build and properties files in your project
