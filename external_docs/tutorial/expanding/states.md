...
[The Supplier, Product, Client and Order objects are complete and linked together](/tutorial/expanding/fieldarea)
...
A state model defines the different statuses a business object can have and controls how it transitions between them.
It enables filtered views, enforces permissions on state changes, and automates actions triggered by transitions.
Setting up a state model ensures a structured workflow for business objects...

[Learn more](/make/businessprocess/state)
...
1. Add a **mandatory enumeration** field to the **TrnOrder** Business object in the
...
2. Fill in the Field fields like so :
   - Label : **State**
   - Logical name : **trnOrdState** _should be auto-completed_
   - Physical name : **trn_ord_state** _should be auto-completed_
   - Required : **Toggled**
...
6. Fill in the List items like so :
   ...
   _Click **Add** to add an element to the List_
...
2. Select the enumeration field that the State model is based on
   ...
   > _By default "trnOrdState" is selected_
3. Click **Next**
...
6. Grant every State transition to the **TRN_SUPERADMIN** Group
   ...
   > The _Associate Action_ checkbox creates a button for each State transition
7. Click **Next**
