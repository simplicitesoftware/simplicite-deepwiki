...
- Backported [from 6.3](/versions/release-notes/v6-3#platform--module-version-check) major version check
  when importing a module: importing modules exported from
...
- Backported [from v6.3](/versions/release-notes/v6-3#mail-service-configuration) support for `MAIL_SERVICE` overriding by the
...
- Backported [from v6.3](/versions/release-notes/v6-3#maven-skip-tests) default modules' Maven `skipTests` to `true` to prevent unit tests to be processed,
  by default, outside of a live instance
- Backported [from v6.3](/versions/release-notes/v6-3#improvements-of-the-generic-openidconnect-provider) improvements of the generic OpenIDConnect provider
- Backported fix on crosstab chart when grouping date per day (`dateGroup='D'`) and field is a datetime
- Backported [from v6.3](/versions/release-notes/v6-3#new-helper-methods) new `getText` helper methods on `PDFTool`
...
- Backported [from v6.3](/versions/release-notes/v6-3#environment-variable-based-configurability) `[ENV:<environment variable name>]`
...
- Backported [from v6.3](/versions/release-notes/v6-3#environment-variable-based-configurability) the possibility to override a system parameter
...
- Backported [from v6.3](/versions/release-notes/v6-3#environment-variable-based-configurability) `[PROP:<JVM property name>]`
  substitutions in module settings and system parameters
- Backported fix on non removed Google font options for CSS declarations (e.g. `Quicksand:wght@300..700` => `Quicksand`)
- Backported [from v6.3](/versions/release-notes/v6-3#improvements-on-external-objects) `getFilename` hook for `ExcelDocumentExternalObject`
  (and legacy `PDFDocumentExternalObject`)
- Backported [from v6.2](/versions/release-notes/v6-2#version-6.2.10) mention of the the `origin` module remote Git URI (if defined)
  to avoid ambiguities vs the instance's module Git URI

### 5.3.72 (2025-06-20) - maintenance revision {#version-5.3.72}

- Backported [from v6.2](/versions/release-notes/v6-2#version-6.2.11) fix on missing image wrapping around `[MODEL:]` tags in module's raw Markdown publication
- Backported [from v6.3](/versions/release-notes/v6-3#custom-disposition-simplification) simplifications for custom dispositions
- Backported [from v6.2](/versions/release-notes/v6-2#version-6.2.11) fix on form-inlined publication buttons styles
- Backported [from v6.2](/versions/release-notes/v6-2#version-6.2.11) fix on module filtering in case of reset and populate foreign keys
- Backported [from v6.2](/versions/release-notes/v6-2#version-6.2.11) module filtering support on groups displayed
  in the _Authorization_ step of the _Create Activity_ system business process
- Backported [from v6.2](/versions/release-notes/v6-2#version-6.2.11) fix on signature pad resizing (`fromDataURL` with `ratio=1`)

### 5.3.73 (2025-06-27) - maintenance revision {#version-5.3.73}

- Backported [from v6.2](/versions/release-notes/v6-2#version-6.2.12) Fix float rendering with progress bar
- Backported [from v6.3](/versions/release-notes/v6-3#custom-disposition-simplification) improvements on custom disposition

### 5.3.74 (2025-07-18) - maintenance revision {#version-5.3.74}

- Backported [from v6.2](/versions/release-notes/v6-2#version-6.2.13) Fix textual crosstab without value axis (count)
- Backported [from v6.2](/versions/release-notes/v6-2#version-6.2.13) fix on follow link on N,N pillbox creation
- Backported [from v6.2](/versions/release-notes/v6-2#version-6.2.13) improvements on platform's "garbage collection" tasks
  to avoid dropping auth-only sessions as "unknown" sessions

### 5.3.75 (2025-07-31) - maintenance revision {#version-5.3.75}

- Backported [from v6.2](/versions/release-notes/v6-2#version-6.2.14) revert on JSON rendering on some system parameters using JSON syntax
  to avoid unordered "random" JSON reformatting
- Backported [from v6.2](/versions/release-notes/v6-2#version-6.2.14) fix XML import to delete the previous `m_document` record in case of empty document
- Backported [from v6.2](/versions/release-notes/v6-2#version-6.2.14) set HTTP status and headers by default in the response
  before calling the `customErrorResponse` platform hook
- Backported [from v6.2](/versions/release-notes/v6-2#version-6.2.14) fix for the issue where the "Use form" setting was ignored for minified lists

### 5.3.76 (2025-08-15) - maintenance revision {#version-5.3.76}

- Backported [from v6.2](/versions/release-notes/v6-2#version-6.2.15) fix tinymce min-height on list edit
- Backported [from v6.2](/versions/release-notes/v6-2#version-6.2.15) fix for placeholders not displayed in action fields
- Backported [from v6.2](/versions/release-notes/v6-2#version-6.2.15) fix do not truncate rich text on validate but return an error
- Backported [from v6.2](/versions/release-notes/v6-2#version-6.2.15) fix for calculated fields eval in history objects
- Backported [from v6.2](/versions/release-notes/v6-2#version-6.2.15) fix row context during bulk update
- Backported [from v6.2](/versions/release-notes/v6-2#version-6.2.15) fix list column order preferences

### 5.3.77 (2025-09-11) - maintenance revision {#version-5.3.77}

- Backported [from v6.2](/versions/release-notes/v6-2#version-6.2.16) fix select2 pillbox style with colored multi-enum
- Backported [from v6.2](/versions/release-notes/v6-2#version-6.2.16) removed "less" button on list edit = extended list only
- Backported [from v6.2](/versions/release-notes/v6-2#version-6.2.16) fix field help on list column

### 5.3.78 (2025-09-30) - maintenance revision {#version-5.3.78}

- Backported [from v6.2](/versions/release-notes/v6-2#version-6.2.17) fix `populateForeignKey` in case of multi-reflexive links
- Backported [from v6.2](/versions/release-notes/v6-2#version-6.2.17) fix for list's column orders when no areas are displayed
- Backported [from v6.2](/versions/release-notes/v6-2#version-6.2.17) fix for issue where the list ordering preference was shown even when the object's
  configuration wasn't relevant (i.e. no field areas visible on the list)
- Backported [from v6.2](/versions/release-notes/v6-2#version-6.2.17) fix on missing standard packages/classes in the public Java API
  (e.g. the annotations package)
- Backported [from v6.3](/versions/release-notes/v6-3#new-helper-methods) `Tool.before/after` date comparison shorthand methods
...
- Backported [from v6.2](/versions/release-notes/v6-2#version-6.2.18) fix on XML import final `status KO` in case of unknown references after recycling
- Backported [from v6.3](/versions/release-notes/v6-3#new-helper-methods) new `Tool.lower/upper/capitalize` null-safe string case manipulation methods
- Backported [from v6.2](/versions/release-notes/v6-2#version-6.2.19) fix on `/io` page in case of non root deployment
...
- Security fixes
- Backported [from v6.2](/versions/release-notes/v6-2#version-6.2.19) reading of the protected `PATCH_LEVEL` system parameter by the ping endpoint
  (instead of the `VERSION` that may be deleted), this avoids potential falsely negative ping responses.
- Backported [from v7.0](/versions/release-notes/v7-0#optional-importspec-version) change on the `version` to be optional
  (if not present no version check is done). Note that the improvement on the Git tag has not been backported.
- Updated  vulnerable JGit Java libs and XTerm additional JS lib
- Backported [from v6.2](/versions/release-notes/v6-2#version-6.2.19) security fixes
- Backported [from v6.2](/versions/release-notes/v6-2#version-6.2.19) missing `initUpdate` on state transition from a row action on list
- Backported [from v6.2](/versions/release-notes/v6-2#version-6.2.19) fix for duplicates of auto-incremented fields

### 5.3.82 (2026-01-19) - maintenance revision {#version-5.3.82}

- Upgraded Java libs
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.0-beta-RC2) fix on
  environment variables / JVM properties substitutions in module's settings
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.0) Apply settings of list of values `FORMAT.NUM` on the big-decimal field

### 5.3.83 (2026-02-06) - maintenance revision {#version-5.3.83}

- Upgraded Java libs
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.1) fix do not `followLink` on html/rich text
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.1) fix on mapped API not returning "raw" JSON for long text fields with JSON rendering
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.3) fix on `Cache-control` HTTP header value for better control on cache management

### 5.3.84 (2026-02-11) - maintenance revision {#version-5.3.84}

- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.4) fix on missing context URL in I/O grant
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.4) added `--legacy-peer-deps` option for `npm install` called from modules' `pom.xml`
- Fixed missing JClouds indirect dependencies

### 5.3.85 (2026-03-11) - maintenance revision {#version-5.3.85}

- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.4) fixed cascade copy of multi-documents field.
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.5) fix JSON body parsing to take into account explicit `null`s.
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.5) addition of missing `com.simplicite.util.engine.DispositionInterface` in public classes.
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.5) fix of potential _not found_ on API tester page depending.
  on the configuration of the `URI_MAPPINGS` system parameter.
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.6) fix of order on `select` objects' enum fields to be by code by default.
...
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.7) fix on location of the `patches` directory.
  and fatal error if this directory is not present during platform upgrade.
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.7) fix on refresh token credentials passing (client ID/client secret).
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.7) fix on `JSONTool.jsonMetaObject` with explicit list of fields.
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.7) fix `Grant.rollbackTransaction()`

### 5.3.87 (2026-04-24) - maintenance revision {#version-5.3.87}

- Upgraded Java libs.
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.8) new log traces for object indexation.
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.8) fix on inappropriate warnings within forgot password feature.
...
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.9) fix on _Register_ form.
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.9) fix on outstanding _I forgot my password_ button after submitting form.
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.9) fix on swallowing `/r` tiny resource URI pattern without `?`.
- Backported [from v6.2](/versions/release-notes/v6-3#) the `customPingResponse` platform hook to customize the `/ping` JSON response if needed.
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.9) fix on useless synchronize when calling custom API external objects on the API
  (`/api/ext`) endpoint.

### 5.3.89 (2026-05-28) - maintenance revision {#version-5.3.89}

- Fixed pagination during links merge
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.10) robustness of populate of nullable key to detect non-`null` empty values.
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.10) added `HTMLTool` methods variants to get HTML/Markdown
  resources contents with URLs substitutions.

### 5.3.90 (2026-06-19) - maintenance revision {#version-5.3.90}

- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.11) fix on email regex to allow legit stacked dashes/plus signs.
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.11) fix on custom action auto-save losing object data when retrying in an error state.

### 5.3.91 (2026-07-10) - maintenance revision {#version-5.3.91}

- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.12) fix on language inconsistency during the _forgot password_ procedure.
...
### 5.3.92 (2026-07-23) - maintenance revision {#version-5.3.92}

- Upgraded Java libs.
- Fixed custom action with error.
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.13) missing branch in the `module-info.json` file

### 5.3.93 (2026-08-28) - maintenance revision {#version-5.3.93}

- Updated Java libs.
- Robustness fixes.

### 5.3.94 (2026-09-09) - maintenance revision {#version-5.3.94}

- Updated Java libs.
- Security fix.
- Backported [from v6.3](/versions/release-notes/v6-3#version-6.3.16) fix for duplicate error messages on hidden fields

### 5.3.95 (UNRELEASED) - maintenance revision {#version-5.3.95}
