...
Make sure you upgrade to the [current minor version](v5-3) of major version 5 (especially if you plan to upgrade to major version 6).
...
This means the recommended JVM to run the platform is still an **up to date JVM**, but **at least** a JVM version 11.
...
- Optimization: fulltext search on PostgreSQL
...
- Optimization: icon picker loads only the visible tabs
...
- Auto-size long-text field (textarea) by default when height is unspecified or precision `h=0`
...
- Editable pivot table:
  ...
  - and user is permitted to edit the data: see edit property on granted groups of pivot tables
...
- Fixed syntax convention proposals on template editor to fallback to previous naming strategy (for objects without syntax conventions prefixes)
...
- Fixed standard JSON indentation
...
- Backported the possibility to pass a dedicated I/O and Git password for the privileged user (`designer`)
...
- Changed: Maven repository is now in "snapshot" mode for all the branches (the Maven version is now `x.y-SNAPSHOT`)
...
- Backported Luhn algorithm check method in `Tool`
...
This is the **final** revision for this minor version 5.0, no further maintenance will be done on it.
